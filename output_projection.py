...
def output_projection_layer(num_units, num_symbols, latent_size, num_embed_units, content_mask, rhetoric_mask, name="output_projection"):
    def output_fn(outputs, latent_z, label_embedding):
        with variable_scope.variable_scope(name):
            ...
            l_fc2 = tf.contrib.layers.fully_connected(l_fc1, 2, activation_fn=None, scope = 'l_fc2')
            p_dis = tf.nn.softmax(l_fc2)
            p_dis_1, p_dis_2 = tf.split(p_dis, 2, axis = 1)
            ...
            p_w = tf.exp(w_fc2)
            p_w = p_w * tf.tile(tf.reshape(content_mask, [1, num_symbols]), [tf.shape(local_d)[0], 1])
            ...
            temp_d2 = temp_d2 * tf.tile(tf.reshape(rhetoric_mask, [1, num_symbols]), [tf.shape(local_d)[0], 1])
            temp_normal = tf.tile(tf.reduce_sum(temp_d2, 1, keep_dims=True), [1, num_symbols])
            y_prob_d2 = tf.div(temp_d2, temp_normal)

            y_prob = p_dis_1 * y_prob_d + p_dis_2 * y_prob_d2
    ...
    def my_sequence_loss(outputs, targets, latent_z, label_embedding, max_time):
        with variable_scope.variable_scope("decoder/%s" % name):
            ...
            l_fc2 = tf.contrib.layers.fully_connected(l_fc1, 2, activation_fn=None, scope = 'l_fc2')
            p_dis = tf.nn.softmax(l_fc2)
            p_dis_1, p_dis_2, p_dis_3 = tf.split(p_dis, 2, axis = 1)
            p_dis_1 = tf.reshape(tf.tile(p_dis_1, [1, num_symbols]), [-1, num_symbols])
            p_dis_2 = tf.reshape(tf.tile(p_dis_2, [1, num_symbols]), [-1, num_symbols])
